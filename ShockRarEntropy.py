...
class EquationClass(EquationBaseClass):
    ...
    def u0(self, x):
        if self.equation_type == "Moving":
            ...
            x1 = x[:, 0].unsqueeze(1)  # 形状: (batch_size, 1)
            x2 = x[:, 1].unsqueeze(1)  # 形状: (batch_size, 1)
    ...
    def interpolate_f(self, file_ex, nu):
        ...
        time_column = exact_solution[condition, 0]  # 时间 t
        space_column = exact_solution[condition, 1]  # 空间 x
    ...
    def exact(self, inputs):
        ...
        if self.equation_type == "Moving":
            ...
            for i in range(inputs.shape[0]):
                t_i = inputs[i, 0].item()  # 提取时间 t
                x_i = inputs[i, 1].item()  # 提取空间 x
                y_i = inputs[i, 2].item()  # 提取 y

                # 如果输入的 t = 0，使用初始条件 u0(x, y)
                if t_i == 0:
                    # 使用 inputs 的后两列 (x, y) 带入初值函数 u0
    ...
    def compute_generalization_error_sine(self, model, images_path=None, num_y_samples=400):
        # 加载精确解数据
        file_ex = "/root/ZHF/cuda0/Data/BurgersExact.txt"
        exact_solution = np.loadtxt(file_ex)

        # 选择标志位为 1.0 且粘度参数为 self.nu 的行
        condition = (exact_solution[:, 2] == 1.0) & (exact_solution[:, 3] == self.nu)

        # 提取精确解并转换为 PyTorch 张量
        Exact_be = torch.tensor(exact_solution[condition, -1].reshape(-1, 1), dtype=torch.float32)

        # 提取时间和空间坐标，转换为 PyTorch 张量
        test_inp_base = torch.tensor(exact_solution[condition, :2], dtype=torch.float32)

        # 在 [-1, 1] 区间内生成均匀采样的 y 值
        y_values = torch.linspace(-1, 1, steps=num_y_samples)
        ...
        # 遍历每个 y_i 进行测试
        for y_i in y_values:
            # 为当前 y_i 生成 (t, x, y_i) 的输入特征
            y_i_column = torch.full((test_inp_base.shape[0], 1), y_i, dtype=torch.float32).to(
                next(model.parameters()).device)
            test_inp = torch.cat((test_inp_base.to(next(model.parameters()).device), y_i_column), dim=1)

            # 将真实解乘以 cos(0.5 * pi * y_i)
            cos_pi_y = np.cos(0.5 * pi * y_i)
            Exact = Exact_be.to(next(model.parameters()).device) * cos_pi_y

            # 模型预测输出
            with torch.no_grad():
                test_out = model(test_inp).cpu().numpy()

            # 计算真实解的绝对值均值
            real_value_mean = np.mean(np.abs(Exact.cpu().numpy()))
            real_value_means.append(real_value_mean)

            # 计算 L2 误差
            L2_test = np.mean(np.abs(Exact.cpu().numpy() - test_out))
            L2_errors.append(L2_test)

        # 计算平均 L2 误差
        L2_error = np.mean(L2_errors)

        # 将 Exact_be 转换为 NumPy 数组以计算相对误差
        Exact_be_np = Exact_be.cpu().numpy()
        rel_L2_error = L2_error / (4 * np.mean(np.abs(Exact_be_np)) / pi)
        print("Error Test:", L2_error)
        print("Relative Error Test:", rel_L2_error)
