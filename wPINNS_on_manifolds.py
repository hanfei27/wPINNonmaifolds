...
def initialize_inputs(len_sys_argv):
    if len_sys_argv == 1:
        ...
        # Additional Info
        folder_path_ = "/root/standing"
        validation_size_ = 0.0
        ensemble_configurations_ = {
            "hidden_layers_sol": 4,
            "hidden_layers_test": 2,
...
if ensemble_configurations["loss_type"] == "l2":
    ...
    raise ValueError
#wandb.init(project="PINN_Burgers", config={"epochs": ensemble_configurations["epochs"]})
...
solution_optimizer = optim.Adam(sol_params, lr=ensemble_configurations["tau_sol"], amsgrad=True)
test_optimizer = optim.Adam(test_params, lr=ensemble_configurations["tau_test"], amsgrad=True)

training_start_time = time.time()

optimization_results = fit(equation_config=Ec,
                          solution_model=solution_model,
                          test_function_model=test_function_model,
                          optimizer_min=solution_optimizer,
                          optimizer_max=test_optimizer,
                          training_set_class=training_set_class,
                          verbose=False)

training_loss_history, optimal_solution_model, optimal_test_model = optimization_results
execution_time = time.time() - training_start_time

visualization_directory = os.path.join(folder_path, "ImagesSol")
os.makedirs(visualization_directory, exist_ok=True)

torch.save(optimal_solution_model, os.path.join(folder_path, "ModelSol.pkl"))
torch.save(optimal_test_model, os.path.join(folder_path, "ModelTest.pkl"))

error_metrics = Ec.compute_generalization_error(optimal_solution_model, visualization_directory)
